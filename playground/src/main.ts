...
const MODULES = [
  ...
  { id: 'compound', icon: '🔗', label: 'Compound Words', color: '#e11d48', count: 4 },
...
const PRESETS: Record<string, { label: string; fields: Record<string, string> }[]> = {
  ...
  'r-detect-compound': [
    ...
    { label: 'بے عزت', fields: { 'dc-text': 'بے عزت آدمی بد نام ہے' } },
    { label: 'Multiple', fields: { 'dc-text': 'کتاب خانہ کا باغ بان خوش قسمت ہے' } },
    { label: 'Izafat و', fields: { 'dc-text': 'علم و عمل ضروری ہے' } },
  ],
  'r-join-compound': [
    { label: 'کتاب خانہ', fields: { 'jc-text': 'کتاب خانہ بہت اچھا ہے' } },
    { label: 'Mixed', fields: { 'jc-text': 'بے عزت آدمی دولت مند نہیں' } },
  ],
}
...
$('app')!.innerHTML = `
  ...
  <div id="mobile-nav">
    ${MODULES.map(m => `
      <button class="mod-btn ${m.id === 'normalization' ? 'active' : ''}" id="mnav-${m.id}" data-module="${m.id}">
...
MODULES.forEach(m => {
  $(`nav-${m.id}`)?.addEventListener('click', () => switchModule(m.id))
  $(`mnav-${m.id}`)?.addEventListener('click', () => switchModule(m.id))
})
...
function switchModule(id: string) {
  ...
  // scroll mobile nav button into view
  $(`mnav-${id}`)?.scrollIntoView({ behavior: 'smooth', block: 'nearest', inline: 'center' })
}
...
function modHeader(tag: string, color: string, icon: string, n: number, title: string, desc: string): string {
  return `
    <div class="mod-header">
      <div class="mod-tag" style="color:${color}">${icon} Module ${n} of 10</div>
...
function panelNormalization(): string {
  return `<div class="module-panel active" id="panel-normalization">
    ...
    <div class="tests-grid">
      ...
      ${card({
      fn: 'normalize', sig: '<span class="t-kw">function</span> normalize(text: <span class="t-type">string</span>, options?: NormalizeOptions): <span class="t-type">string</span>',
      ...
      ${card({
      fn: 'fingerprint', sig: '<span class="t-kw">function</span> fingerprint(text: <span class="t-type">string</span>): <span class="t-type">string</span>',
      ...
      ${card({
      fn: 'stripDiacritics', sig: '<span class="t-kw">function</span> stripDiacritics(text: <span class="t-type">string</span>): <span class="t-type">string</span>',
      ...
      ${card({
      fn: 'normalizeCharacters', sig: '<span class="t-kw">function</span> normalizeCharacters(text: <span class="t-type">string</span>): <span class="t-type">string</span>',
      ...
      ${card({
      fn: 'stripZeroWidth', sig: '<span class="t-kw">function</span> stripZeroWidth(text: <span class="t-type">string</span>): <span class="t-type">string</span>',
      ...
      ${card({
      fn: 'normalizeAlif', sig: '<span class="t-kw">function</span> normalizeAlif(text: <span class="t-type">string</span>): <span class="t-type">string</span>',
      ...
      ${card({
      fn: 'normalizeNumerals', sig: '<span class="t-kw">function</span> normalizeNumerals(text: <span class="t-type">string</span>): <span class="t-type">string</span>',
...
function panelSearch(): string {
  return `<div class="module-panel" id="panel-search">
    ...
    <div class="tests-grid">
      ...
      ${card({
      fn: 'match', sig: '<span class="t-kw">function</span> match(query: <span class="t-type">string</span>, target: <span class="t-type">string</span>): MatchResult | <span class="t-type">null</span>',
      ...
      ${card({
      fn: 'fuzzyMatch', sig: '<span class="t-kw">function</span> fuzzyMatch(query: <span class="t-type">string</span>, candidates: <span class="t-type">string</span>[], threshold?: <span class="t-type">number</span>): <span class="t-type">string</span>[]',
      ...
      ${card({
      fn: 'getAllNormalizations', sig: '<span class="t-kw">function</span> getAllNormalizations(word: <span class="t-type">string</span>): <span class="t-type">string</span>[]',
...
function panelNumbers(): string {
  return `<div class="module-panel" id="panel-numbers">
    ...
    <div class="tests-grid">
      ...
      ${card({
      fn: 'numberToWords', sig: '<span class="t-kw">function</span> numberToWords(n: <span class="t-type">bigint</span>, opts?: WordOpts): <span class="t-type">string</span>',
      ...
      ${card({
      fn: 'formatCurrency', sig: '<span class="t-kw">function</span> formatCurrency(amount: <span class="t-type">number</span>, currency: <span class="t-str">"PKR"</span> | <span class="t-str">"INR"</span>): <span class="t-type">string</span>',
      ...
      ${card({
      fn: 'wordsToNumber', sig: '<span class="t-kw">function</span> wordsToNumber(text: <span class="t-type">string</span>): <span class="t-type">bigint</span>',
      ...
      ${card({
      fn: 'toUrduNumerals', sig: '<span class="t-kw">function</span> toUrduNumerals(text: <span class="t-type">string</span>): <span class="t-type">string</span>',
...
function panelTokenization(): string {
  return `<div class="module-panel" id="panel-tokenization">
    ...
    <div class="tests-grid">
      ...
      ${card({
      fn: 'tokenize', sig: '<span class="t-kw">function</span> tokenize(text: <span class="t-type">string</span>): Token[]',
      ...
      body: `
          ...
          <div class="checks-row">
            <label class="ctrl-check"><input type="checkbox" id="tok-compound" checked />Join Compounds Before Tokenizing</label>
          </div>
          ${runBtn('Tokenize', 'btn-tokenize')}`
    })}

      ${card({
      fn: 'sentences', sig: '<span class="t-kw">function</span> sentences(text: <span class="t-type">string</span>): <span class="t-type">string</span>[]',
      ...
      ${card({
      fn: 'ngrams', sig: '<span class="t-kw">function</span> ngrams(tokens: <span class="t-type">string</span>[], n: <span class="t-type">number</span>): <span class="t-type">string</span>[][]',
...
function panelCompound(): string {
  return `<div class="module-panel" id="panel-compound">
    ${modHeader('compound', '#e11d48', '🔗', 5, 'Compound Words',
    `UAWL affix-based + izafat marker detection. Detects multi-word semantic units like کتاب خانہ, بے عزت,
       علم و عمل. Joins with ZWNJ (U+200C) so naive tokenizers treat compounds as single tokens. Round-trip safe.`)}
    <div class="tests-grid">

      ${card({
      fn: 'detectCompounds', sig: '<span class="t-kw">function</span> detectCompounds(text: <span class="t-type">string</span>, options?: CompoundOptions): CompoundSpan[]',
      badge: 'Affix + Izafat', color: '#e11d48', importFn: 'detectCompounds', resultId: 'r-detect-compound',
      desc: 'Detects compound word spans using UAWL affix matching and izafat marker detection. Returns typed spans with component words and detection method.',
      presets: PRESETS['r-detect-compound'],
      body: `
          ${inputRow('dc-text', 'Urdu text', 'کتاب خانہ بہت اچھا ہے', 'urdu', 'textarea')}
          <div class="checks-row">
            <label class="ctrl-check"><input type="checkbox" id="dc-affix" checked />Affix (UAWL)</label>
            <label class="ctrl-check"><input type="checkbox" id="dc-izafat" checked />Izafat (zer/و)</label>
          </div>
          ${runBtn('Detect Compounds', 'btn-detect-compound')}`
    })}

      ${card({
      fn: 'joinCompounds', sig: '<span class="t-kw">function</span> joinCompounds(text: <span class="t-type">string</span>, options?: CompoundOptions): <span class="t-type">string</span>',
      badge: 'Space → ZWNJ', color: '#e11d48', importFn: 'joinCompounds', resultId: 'r-join-compound',
      desc: 'Detects compound words and replaces the space between them with a binder character (default: ZWNJ U+200C). Downstream tokenizers will treat the compound as one token.',
      presets: PRESETS['r-join-compound'],
      body: `
          ${inputRow('jc-text', 'Urdu text', 'کتاب خانہ بہت اچھا ہے', 'urdu', 'textarea')}
          <select class="ctrl-select" id="jc-binder">
            <option value="zwnj">ZWNJ (U+200C) — default</option>
            <option value="nbsp">NBSP (U+00A0)</option>
            <option value="wj">Word Joiner (U+2060)</option>
          </select>
          ${runBtn('Join Compounds', 'btn-join-compound')}`
    })}

      ${card({
      fn: 'splitCompounds', sig: '<span class="t-kw">function</span> splitCompounds(text: <span class="t-type">string</span>): <span class="t-type">string</span>',
      badge: 'ZWNJ → Space', color: '#e11d48', importFn: 'splitCompounds', resultId: 'r-split-compound',
      desc: 'Reverses joinCompounds() — replaces ZWNJ, NBSP, and Word Joiner characters between words back to regular spaces. Round-trip safe.',
      body: `
          ${inputRow('sc-text', 'Text with binders (paste joined output)', 'کتاب\u200Cخانہ بہت اچھا ہے', 'urdu', 'textarea')}
          ${runBtn('Split Compounds', 'btn-split-compound')}`
    })}

      ${card({
      fn: 'isCompound', sig: '<span class="t-kw">function</span> isCompound(w1: <span class="t-type">string</span>, w2: <span class="t-type">string</span>): CompoundMatch',
      badge: 'Pair Check', color: '#e11d48', importFn: 'isCompound', resultId: 'r-is-compound',
      desc: 'Checks whether two adjacent words form a compound. Returns the detection method (affix/izafat) or null if not a compound. Use for custom compound logic.',
      body: `
          ${inputRow('ic-w1', 'Word 1', 'کتاب', 'urdu')}
          ${inputRow('ic-w2', 'Word 2', 'خانہ', 'urdu')}
          ${runBtn('Check Pair', 'btn-is-compound')}`
    })}
    </div>
  </div>`
}
...
function panelStringUtils(): string {
  return `<div class="module-panel" id="panel-string-utils">
    ${modHeader('string-utils', '#14b8a6', '📝', 6, 'String Utilities',
    ...
    <div class="tests-grid">
      ...
      ${card({
      fn: 'reverse', sig: '<span class="t-kw">function</span> reverse(text: <span class="t-type">string</span>): <span class="t-type">string</span>',
      ...
      ${card({
      fn: 'truncate', sig: '<span class="t-kw">function</span> truncate(text: <span class="t-type">string</span>, length: <span class="t-type">number</span>, suffix?: <span class="t-type">string</span>): <span class="t-type">string</span>',
      ...
      ${card({
      fn: 'wordCount / charCount', sig: '<span class="t-kw">function</span> wordCount(text): <span class="t-type">number</span> | charCount(text): <span class="t-type">number</span>',
      ...
      ${card({
      fn: 'extractUrdu', sig: '<span class="t-kw">function</span> extractUrdu(text: <span class="t-type">string</span>): <span class="t-type">string</span>[]',
      ...
      ${card({
      fn: 'decodeHtmlEntities', sig: '<span class="t-kw">function</span> decodeHtmlEntities(html: <span class="t-type">string</span>): <span class="t-type">string</span>',
      ...
      ${card({
      fn: 'pad', sig: '<span class="t-kw">function</span> pad(text: <span class="t-type">string</span>, length: <span class="t-type">number</span>, char?: <span class="t-type">string</span>, dir?: <span class="t-str">"start"</span>|<span class="t-str">"end"</span>): <span class="t-type">string</span>',
...
function panelEncoding(): string {
  return `<div class="module-panel" id="panel-encoding">
    ${modHeader('encoding', '#fb923c', '💾', 7, 'Encoding',
    ...
    <div class="tests-grid">
      ...
      ${card({
      fn: 'detectEncoding', sig: '<span class="t-kw">function</span> detectEncoding(buffer: Uint8Array): Encoding',
      ...
      ${card({
      fn: 'decodeInpage', sig: '<span class="t-kw">function</span> decodeInpage(buffer: Uint8Array, version: InpageVersion): InpageDecodeResult',
      ...
      ${card({
      fn: 'convertWindows1256ToUnicode', sig: '<span class="t-kw">function</span> convertWindows1256ToUnicode(text: <span class="t-type">string</span>): <span class="t-type">string</span>',
...
function panelSorting(): string {
  return `<div class="module-panel" id="panel-sorting">
    ${modHeader('sorting', '#6366f1', '🔤', 8, 'Urdu Sorting',
    ...
    <div class="tests-grid">
      ...
      ${card({
      fn: 'sort', sig: '<span class="t-kw">function</span> sort(words: <span class="t-type">string</span>[], reverse?: <span class="t-type">boolean</span>): <span class="t-type">string</span>[]',
      ...
      ${card({
      fn: 'compare / sortKey', sig: '<span class="t-kw">function</span> compare(a: <span class="t-type">string</span>, b: <span class="t-type">string</span>): <span class="t-type">number</span>',
...
function panelTransliteration(): string {
  return `<div class="module-panel" id="panel-transliteration">
    ${modHeader('transliteration', '#f43f5e', '🔄', 9, 'Transliteration',
    ...
    <div class="tests-grid">
      ...
      ${card({
      fn: 'toRoman', sig: '<span class="t-kw">function</span> toRoman(text: <span class="t-type">string</span>): <span class="t-type">string</span>',
      ...
      ${card({
      fn: 'fromRoman', sig: '<span class="t-kw">function</span> fromRoman(text: <span class="t-type">string</span>): <span class="t-type">string</span>',
...
function panelAnalysis(): string {
  return `<div class="module-panel" id="panel-analysis">
    ${modHeader('analysis', '#a855f7', '🧬', 10, 'Text Analysis',
    ...
    <div class="tests-grid">
      ...
      ${card({
      fn: 'getScript / isRTL', sig: '<span class="t-kw">function</span> getScript(text: <span class="t-type">string</span>): ScriptFamily',
      ...
      ${card({
      fn: 'getUrduDensity', sig: '<span class="t-kw">function</span> getUrduDensity(text: <span class="t-type">string</span>): <span class="t-type">number</span>',
      ...
      ${card({
      fn: 'classifyChar', sig: '<span class="t-kw">function</span> classifyChar(cp: <span class="t-type">number</span>): CharClass',
      ...
      ${card({
      fn: 'isUrduChar', sig: '<span class="t-kw">function</span> isUrduChar(char: <span class="t-type">string</span>): <span class="t-type">boolean</span>',
...
wire('btn-numerals', () => {
  try {
    const t = $v('num-text'), r = normalizeNumerals(t, 'ascii')
...
wire('btn-wtn', () => {
  try {
    ...
    if (result === null) {
      setError('r-wtn', 'Could not parse number')
      return
    }
...
wire('btn-classify', () => {
  try {
    ...
    setResultHTML('r-classify', `
      ...
      </table>`, chars.map(c => `${c}:${classifyChar(c)}`).join(' '))
...
wire('btn-detect-compound', () => {
  try {
    const text = $v('dc-text')
    const spans = detectCompounds(text, {
      affix: $checked('dc-affix'),
...
wire('btn-join-compound', () => {
  try {
    const text = $v('jc-text')
    const binder = $v('jc-binder') as 'zwnj' | 'nbsp' | 'wj'
    const result = joinCompounds(text, { binder })
    const changed = result !== text
    const binderName = binder === 'zwnj' ? 'ZWNJ U+200C' : binder === 'nbsp' ? 'NBSP U+00A0' : 'WJ U+2060'
    setResultHTML('r-join-compound', `
      <div class="ba-box">
        <div class="ba-side"><div class="ba-label">Before</div><div class="ba-val">${esc(text)}</div><div class="result-meta">${toUniEsc(text)}</div></div>
        <div class="ba-arrow">→</div>
        <div class="ba-side"><div class="ba-label">After</div><div class="ba-val" style="color:${changed ? 'var(--green)' : 'var(--text-2)'}">${esc(result)}</div><div class="result-meta">${toUniEsc(result)}</div></div>
      </div>
      <div class="result-meta" style="margin-top:8px">${changed ? `✓ Joined with ${binderName}` : '● No compounds found'}</div>`, result)
  } catch (e) { setError('r-join-compound', String(e)) }
})

wire('btn-split-compound', () => {
  try {
    const text = $v('sc-text')
    const result = splitCompounds(text)
    const changed = result !== text
    setResultHTML('r-split-compound', `
      <div class="ba-box">
        <div class="ba-side"><div class="ba-label">Before</div><div class="ba-val">${esc(text)}</div><div class="result-meta">${toUniEsc(text)}</div></div>
        <div class="ba-arrow">→</div>
        <div class="ba-side"><div class="ba-label">After</div><div class="ba-val" style="color:${changed ? 'var(--green)' : 'var(--text-2)'}">${esc(result)}</div><div class="result-meta">${toUniEsc(result)}</div></div>
      </div>
      <div class="result-meta" style="margin-top:8px">${changed ? '✓ Binders restored to spaces' : '● No binder characters found'}</div>`, result)
  } catch (e) { setError('r-split-compound', String(e)) }
})

wire('btn-is-compound', () => {
  try {
