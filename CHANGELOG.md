...
### Fixed
- Stripped unsupported HTML tags from the `README.md` to fix broken documentation rendering on NuGet's gallery.
- Fixed the `pages.yml` GitHub action to ensure the Live Playground deploys successfully.


## [1.4.0] – 2026-04-30

### Changed
- **Unification:** The NPM and NuGet packages have now been strictly unified. The package will appear everywhere as `urdu-tools` (NPM, GitHub) and `Urdu-Tools` (NuGet).
- Fixed NPM publishing pipeline crash caused by a naming collision between the monorepo root and the NPM workspace package.

## [1.3.2] – 2026-04-30

### Fixed
- Fixed NPM package publish to properly include the `README.md` file (by adding a `prepack` copy script).
- Kept the internal C# namespace and directory as `UrduTools.Core` for stability, while safely publishing the NuGet package as `UrduTools`.
